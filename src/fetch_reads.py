...
def print_if_ok(sr={}):
	outputfmt = "%(chr)s\t%(split_position)s\t%(prime)s\t%(length)s\t%(seq)s\t%(qual)s\t%(mapq)s\t%(avg_sr_qual).2f"
	if sr["length"] >= MINIMUM_LENGTH_SPLIT_READ and sr["mapq"] >= MINIMUM_MAPQ and sr["avg_sr_qual"] >= MININUM_AVERAGE_BASE_QUALITY_SR:
		print(outputfmt % sr)
...
print "chr\tsplit_position\tprime\tsplit_length\tseq\tqual\tmapq\tavg_sr_qual"
