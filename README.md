...
* `--i` : path to the input file (output of score command after running the [score](#3-score) command).
* `--o` : path to output the annotated file.
* `--d` : path to the InDelible frequency database generated by the [database](#4-database) step.
* `--config` : path to the config.yml file.
